...
$syncHash.Find_StartFind.Add_Click( {
    if ($global:RSJobCleanup) {
        [void]$global:RSJobCleanup.PowerShell.Stop()
        [void]$global:RSJobCleanup.PowerShell.Dispose()
    }
...
$syncHash.Main_Add.Add_Click({ 
    Update-Field -ControlName Main_Result -SyncHash $syncHash -Value "Adding printer to $($configHash.printerSettings[0].PrintServer)..." -Property Text 

    $global:PrintSettings = @{
        ComputerName  = $configHash.printerSettings[0].PrintServer
        Name          = $SyncHash.Prop_Name.Text
        Location      = $SyncHash.Prop_Location.Text
        Driver        = $configHash.printerSettings[0].PrinterDriver
        PortName      = $configHash.printerSettings[0].PrinterPortName
        Published     = $configHash.printerSettings[0].PropPublishADDS
        Shared        = $true
        ShareName     = $SyncHash.Prop_Name.Text
        RenderingMode = if ($configHash.printerSettings[0].PropServerSpool) { 'SSR' } else { 'CSR' }
        Comment       = if ($configHash.printerSettings[0].PropAppendEWS -and $PrintSettings.PrinterURL) { $PrintSettings.PropComments + '--' + $PrintSettings.PrinterURL }
        else { $configHash.printerSettings[0].PropComments }
    }

    New-RS -ScriptBlock { Add-PrinterToServer -PrintSettings $PrintSettings } -RunspaceName AddPrinter
